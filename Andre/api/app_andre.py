...
def get_templos():
    ...
    try:
        ...
        if id_param:
            ...
            templo_data = cursor.fetchone() # Busca um único registro
            if not templo_data:
                # Retorna 404 se o templo não for encontrado
                return jsonify({"error": "Templo não encontrado."}), 404

            # Formata as datas para o templo individual (se existirem)
            if templo_data.get('DATA_ABERTURA_TEMPLO'):
                templo_data['DATA_ABERTURA_TEMPLO'] = templo_data['DATA_ABERTURA_TEMPLO'].strftime('%Y-%m-%d')
            if templo_data.get('DATA_FECHAMENTO_TEMPLO'):
                templo_data['DATA_FECHAMENTO_TEMPLO'] = templo_data['DATA_FECHAMENTO_TEMPLO'].strftime('%Y-%m-%d')
            
            # Retorna o objeto do templo diretamente, como o frontend espera
            return jsonify(templo_data)

        # Lógica existente para 'all' ou paginado
        ...
        # Formata as datas para a lista de templos
        for templo_item in templos: # Use um nome de variável diferente para evitar conflito
            if templo_item.get('DATA_ABERTURA_TEMPLO'):
                templo_item['DATA_ABERTURA_TEMPLO'] = templo_item['DATA_ABERTURA_TEMPLO'].strftime('%Y-%m-%d')
            if templo_item.get('DATA_FECHAMENTO_TEMPLO'):
                templo_item['DATA_FECHAMENTO_TEMPLO'] = templo_item['DATA_FECHAMENTO_TEMPLO'].strftime('%Y-%m-%d')
        ...
        # Se ocorrer um erro inesperado, logar e retornar 500
        print(f"Erro na rota /api/templos: {e}") # Adicionado para depuração no servidor
        return jsonify({"error": "Ocorreu um erro interno no servidor."}), 500 # Mensagem mais genérica para o usuário
    finally:
